...
Now, we can try to overlay the histogram plot using a lognormal density with the estimates for ***location*** and ***scale*** using respectively the posterior mean and the lower and upper bounds of the 95% credible intervals.
...
lines(seq(0,1.5,0.01), dlnorm(seq(0,1.5,0.01),  -1.396, 0.9376), lwd = 3, 
      col = "red")
lines(seq(0,1.5,0.01), dlnorm(seq(0,1.5,0.01),  -1.114, 1.139), lwd = 3, 
      col = rgb(.14,1,0,.5))
legend("topright", legend = c("logNormal~(-1.255,1.032)", "logNormal~(-1.396,0.9376)", "logNormal~(-1.114,1.139)"), 
       lwd = 3, col = c("orange", "red", rgb(.14,1,0,.5)), bty = 'n')
